...
(6, 'abc', 'large', 101, '2020-03-19', 12333, 2),
...
INSERT INTO `task_table` (`task_id`, `task_action`, `task_status`, `emp_id`) VALUES
(1, 'packing', 'in progress', 1),
(7, 'shipping', 'started', 3),
(8, 'in progress', 'packing', 1),
(10, 'in progress', 'packing', 3);
...
ALTER TABLE `task_table`
  ...
  ADD KEY `task_table_ibfk_1` (`emp_id`);
...
--
ALTER TABLE `task_emptable`
  MODIFY `task_emp_id` int(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
...
--
ALTER TABLE `task_table`
  ADD CONSTRAINT `task_table_ibfk_1` FOREIGN KEY (`emp_id`) REFERENCES `employee_table` (`emp_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;
